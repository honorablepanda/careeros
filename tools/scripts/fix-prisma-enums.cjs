...
function resolveTargets() {
  ...
  return [path.join('apps', 'api', 'src', 'trpc', 'routers', 'tracker.router.ts')];
}
...
function patchContent(s) {
  ...
  s = s.replace(
    ...
    (m, g1) => { changed = true; return `import { ${g1} } from '@prisma/client';`; }
  );
  ...
  if (!/from '@prisma\/client'/.test(s)) {
    ...
    s = s.replace(
      ...
      (m, g1) => {
        let names = g1.split(',').map(v => v.trim()).filter(Boolean);
...
function patchFile(p) {
  if (!fs.existsSync(p)) { console.log(`(skip) ${p} not found`); return 0; }
  const orig = fs.readFileSync(p, 'utf8');
  const { s, changed } = patchContent(orig);
  if (!changed) { console.log(`= No changes: ${p}`); return 0; }
...
let modified = 0;
targets.forEach(f => { modified += patchFile(f); });
