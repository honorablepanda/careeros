...
function report(file) {
  ...
  const labelOk =
    /<label[^>]*\bhtmlFor="statusFilter"[^>]*>\s*Filter status:\s*<\/label>/.test(
      s
    );
  console.log(
    labelOk
      ? 'PASS: label htmlFor="statusFilter"'
      : 'FAIL: label htmlFor="statusFilter"'
  );
  ...
  const arrowOk =
    /onChange=\{\s*\(e\)\s*=>\s*setStatus\(\s*e\.target\.value\s*\|\|\s*undefined\s*\)\s*\}/.test(
      openTag
    );

  console.log(idOk ? 'PASS: id="statusFilter"' : 'FAIL: id="statusFilter"');
  console.log(
    valOk ? "PASS: value={status ?? ''}" : "FAIL: value={status ?? ''}"
  );
  ...
  console.log(
    !idInside ? 'PASS: no id inside onChange' : 'FAIL: id found inside onChange'
  );
  console.log(
    !downgrad ? 'PASS: no downgraded "(e) ="' : 'FAIL: downgraded "(e) ="'
  );
  ...
  return labelOk && idOk && valOk && arrowOk && !idInside && !downgrad && !stray
    ? 0
    : 1;
}
