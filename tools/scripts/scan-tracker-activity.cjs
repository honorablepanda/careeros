...
const EXCLUDES = new Set(['node_modules', '.git', '.next', 'dist', 'build', '.turbo', 'out']);
...
if (!routerFile) {
  ...
  const hasGetActivity = /getApplicationActivity\s*:\s*publicProcedure/.test(norm);
  if (hasGetActivity) {
    ok('getApplicationActivity: symbol present');
    const m = /applicationActivity\s*\.\s*findMany\s*\(\s*\{[\s\S]*?where\s*:\s*\{\s*applicationId\s*:\s*input\.id\s*\}[\s\S]*?orderBy\s*:\s*\{\s*createdAt\s*:\s*'desc'\s*\}[\s\S]*?\}\s*\)/m.exec(norm);
    if (m) {
      const idx = m.index;
      ok('getApplicationActivity: calls prisma.applicationActivity.findMany with orderBy createdAt desc');
      log(dim(contextSnippet(norm, idx)));
    } else {
      warn('getApplicationActivity: could not verify prisma call + orderBy createdAt desc (pattern not found).');
    }
  } else {
  ...
  const createBlock = /createApplication\s*:\s*publicProcedure[\s\S]*?\.mutation\([\s\S]*?\{([\s\S]*?)\}\s*\)\s*,?/m.exec(norm);
  if (!createBlock) {
    warn('createApplication: block not found (name or structure differs).');
  } else {
    const body = createBlock[1];
    const hasCreate = /applicationActivity\s*\.\s*create\s*\([\s\S]*type\s*:\s*['"]CREATED['"]/.test(body);
    if (hasCreate) {
      ok('createApplication: writes activity type "CREATED".');
      log(dim(contextSnippet(body, body.search(/applicationActivity\s*\.\s*create/))));
    } else {
  ...
  const updateBlock = /updateApplication\s*:\s*publicProcedure[\s\S]*?\.mutation\([\s\S]*?\{([\s\S]*?)\}\s*\)\s*,?/m.exec(norm);
  if (!updateBlock) {
    warn('updateApplication: block not found (name or structure differs).');
  } else {
    const body = updateBlock[1];
    const hasStatusChange = /applicationActivity\s*\.\s*create\s*\([\s\S]*type\s*:\s*['"]STATUS_CHANGE['"]/.test(body);
    if (hasStatusChange) {
      ok('updateApplication: writes activity type "STATUS_CHANGE" (status change).');
      log(dim(contextSnippet(body, body.search(/applicationActivity\s*\.\s*create/))));
    } else {
...
if (!schemaFile) {
  ...
  const modelMatch = /model\s+ApplicationActivity\s*\{([\s\S]*?)\}/m.exec(schema || '');
  if (!modelMatch) {
    warn('No "model ApplicationActivity" in schema.prisma (router will gracefully no-op).');
  } else {
    ...
    if (hasAppId && hasCreatedAt && hasType) {
      ok('ApplicationActivity: has applicationId, createdAt, type (basic fields present).');
    } else {
      warn('ApplicationActivity: missing one of applicationId/createdAt/type fields.');
    }
...
if (!pageFile) {
  ...
  const present = tsconfigs.filter(t => t.j).map(t => {
    try { return { p: t.p, json: JSON.parse(t.j) }; } catch { return null; }
  }).filter(Boolean);
  ...
  for (const t of present) {
    ...
    log(dim(`tsconfig: ${path.relative(ROOT, t.p)} paths keys: ${Object.keys(paths).join(', ') || '(none)'}`));
  }
  if (hasAlias) ok('tsconfig paths includes "@/*" -> "norm/*" (or similar).');
  else warn('tsconfig paths does NOT include "@/*" mapping; alias may fail to resolve.');
  ...
  if (foundTrpc) ok(`TRPC client file found -> ${path.relative(ROOT, foundTrpc)}`);
  else warn('No TRPC client file found under web/norm/trpc/*.ts (alias will fail).');
}
...
const notes = [];
function collect(line){ notes.push(line); console.log(line); }
...
console.log('\n' + cyan('Done. If any FAIL/WARN appear above, they are the exact fixes tests are expecting.'));
