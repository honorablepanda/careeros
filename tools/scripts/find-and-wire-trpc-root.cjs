...
function walk(dir, out = []) {
  for (const ent of fs.readdirSync(dir, { withFileTypes: true })) {
    if (['node_modules', '.git', '.next', 'dist', 'build', 'coverage', 'out'].includes(ent.name)) continue;
...
function toImport(fromDir, toFile) {
  return './' + path.relative(fromDir, toFile).replace(/\\/g, '/').replace(/\.(t|j)sx?$/, '');
}

const files = walk(ROOT);

// find tracker.router.*
const trackerFile = files.find(p => /[/\\]tracker\.router\.(t|j)sx?$/i.test(p));
if (!trackerFile) {
...
const APP_TYPE_RE = /export\s+type\s+AppRouter\s*=\s*typeof\s+([A-Za-z0-9_]+)\s*;/m;

const candidates = files
  .map(f => ({ file: f, src: read(f) }))
  .filter(({ src, file }) =>
    APP_TYPE_RE.test(src) || /(createTRPCRouter|t\.router|mergeRouters|t\.mergeRouters)\s*\(/.test(src)
  )
  .map(({ file, src }) => {
...
function ensureImport(src, importName, importPath) {
  const already = new RegExp(`import\\s*{[^}]*\\b${importName}\\b[^}]*}\\s*from\\s*['"][^'"]*tracker\\.router`, 'm').test(src) ||
                  new RegExp(`import\\s*{[^}]*\\b${importName}\\b[^}]*}\\s*from\\s*['"]${importPath.replace(/[.*+?^${}()|[\]\\]/g,'\\$&')}['"]`, 'm').test(src);
  if (already) return { src, added:false };
  const allImports = [...src.matchAll(/^\s*import[\s\S]*?;$/gm)];
  const insertAt = allImports.length ? allImports[allImports.length-1].index + allImports[allImports.length-1][0].length : 0;
  const line = `\nimport { trackerRouter } from '${importPath}';\n`;
  return { src: src.slice(0, insertAt) + line + src.slice(insertAt), added:true };
}

const EXPORT_ASSIGN_RE = /export\s+(?:const|let|var)\s+appRouter\s*=\s*([\s\S]*?);/m;
const REEXPORT_RE = /export\s*{\s*appRouter\s*(?:as\s+[A-Za-z0-9_]+)?\s*}\s*from\s*['"]([^'"]+)['"];?/m;
...
function findExportAssignment(filePath, seen = new Set()) {
  ...
  if (r) {
    const target = r[1];
    const abs = path.resolve(path.dirname(filePath), target.replace(/\.(t|j)sx?$/,''));
    const withExt = ['.ts', '.tsx', '.js', '.jsx'].map(ext => abs + ext).find(p => fs.existsSync(p));
...
function alreadyHasTracker(rhsOrSrc) {
  return /tracker\s*:\s*trackerRouter/.test(rhsOrSrc) ||
         /\.merge\s*\(\s*['"]tracker\.\s*['"]\s*,\s*trackerRouter\s*\)/.test(rhsOrSrc) ||
         /\btrackerRouter\b/.test(rhsOrSrc) && /\.merge\s*\(/.test(rhsOrSrc);
}

function patchAssignmentBlock(src, assign) {
  if (alreadyHasTracker(assign.full)) return { src, changed:false, method:'already-wired' };
...
function processOne(absPath) {
  ...
  if (!assign) return { error: 'Could not locate `export const appRouter = ...;` (directly or via re-export).' };
  ...
  if (!reAssign) return { error: 'Lost appRouter assignment after import insertion (parser issue).' };
  ...
  return { importAdded: imp.added, changed: patched.changed, method: patched.method };
}

// -------- run
if (candidates.length === 0) {
  console.log('No tRPC roots found.\nTip: look for `export type AppRouter = typeof appRouter` or `createTRPCRouter(...)`.');
  process.exit(0);
}

console.log('tRPC root candidates (highest confidence first):\n');
for (const c of candidates.slice(0, 10)) {
  console.log(`→ ${rel(c.file)}  ${c.varName ? `(AppRouter → ${c.varName})` : ''}`);
...
for (const t of targets) {
  ...
  if (!owner) {
    console.log(`${rel(t.file)}  ERROR: Could not locate \`export const appRouter = ...\` (directly or via re-export).`);
    continue;
  }
  const res = processOne(owner.filePath);
  if (res.error) console.log(`${rel(owner.filePath)}  ERROR: ${res.error}`);
  else console.log(`${rel(owner.filePath)}  changes:${res.importAdded?' +import':''}${res.changed?' +wire':''} [method=${res.method}]`);
}
