...
function hasGh(){ try { sh('gh --version'); return true; } catch { return false; } }
...
function viewRelease() {
  try {
    return JSON.parse(
      sh(`gh release view ${tag} --repo ${repo} --json tagName,url,isDraft,isPrerelease,assets,body`)
    );
  } catch { return null; }
}
...
if (!hasHealth || !hasTrpc) {
  console.error('[verify-release] Missing expected assets. Saw:', names.join(', '));
  process.exit(1);
}
