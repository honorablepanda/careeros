...
const FAIL_AFTER =
  (args.find((a) => a.startsWith('--fail-after=')) || '').split('=')[1] || null;
...
function detectStub(src) {
  ...
  if (src.startsWith('/** STUB:PHASE3'))
    return { stub: true, reason: 'STUB:PHASE3 header' };
  if (
    /Minimal placeholder router|export const\s+\w+Router\s*=\s*\{\}\s*as any;/.test(
      src
    )
  )
  ...
  if (
    /export type\s+[A-Z][A-Za-z0-9]*DTO/.test(src) &&
    src.split('\n').length < 60
  )
...
(function main() {
  ...
  for (const r of rows) {
    md += `| ${r.module} | ${r.kind} | ${r.status} | ${r.reason || ''} | ${
      r.path
    } |\n`;
  }
  ...
  if (FAIL || FAIL_AFTER) {
    ...
    if (shouldFail) {
      console.error(
        `✗ Stubs present${cutoff ? ` (past ${FAIL_AFTER})` : ''}: ${
          summary.stubs
        }`
      );
      process.exitCode = 2;
