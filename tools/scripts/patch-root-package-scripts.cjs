...
const R=root();
const pkgPath=path.join(R,'package.json'); if(!fs.existsSync(pkgPath)){console.error('package.json not found');process.exit(1)}

function detectApiDir(){
  if(fs.existsSync(path.join(R,'api','jest.config.ts'))) return 'api';
  if(fs.existsSync(path.join(R,'apps','api','jest.config.ts'))) return 'apps/api';
  return 'api';
}
...
j.scripts['test:api'] = `pnpm -w jest --clearCache --config ${jestCfg} && pnpm -w exec nx run api:test --verbose`;
j.scripts['test:api:guarded'] = 'node tools/scripts/guard-jest-config.cjs && pnpm run test:api';
