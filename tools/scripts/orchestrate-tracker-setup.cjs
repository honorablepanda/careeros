...
function detectApiDir(){
  if(fs.existsSync(path.join(R,'api','project.json')) || fs.existsSync(path.join(R,'api','jest.config.ts'))) return 'api';
  if(fs.existsSync(path.join(R,'apps','api','project.json')) || fs.existsSync(path.join(R,'apps','api','jest.config.ts'))) return 'apps/api';
  throw new Error('Cannot find API project (api/ or apps/api).');
}
function detectSchema(){
  if(fs.existsSync(path.join(R,'shared','prisma','schema.prisma'))) return 'shared/prisma/schema.prisma';
  if(fs.existsSync(path.join(R,'prisma','schema.prisma'))) return 'prisma/schema.prisma';
...
if(!has(rootPkg,'prisma')) run('pnpm -w add -D prisma'); else console.log('[deps] prisma ok at root');
...
if(fs.existsSync(sharedPrismaPkg)) {
  ...
  if(!has(sp,'@prisma/client')) run('pnpm --filter ./shared/prisma add @prisma/client'); else console.log('[deps] @prisma/client ok in shared/prisma');
} else {
  if(!has(rootPkg,'@prisma/client')) run('pnpm -w add @prisma/client'); else console.log('[deps] @prisma/client ok at root');
}
...
const rootTsCands=[path.join(R,apiDir,'src','trpc','root.ts'),path.join(R,apiDir,'src','trpc','root','index.ts')].filter(fs.existsSync);
...
if(!SKIP){
  ...
  try { run('pnpm -w exec nx run-many -t build --all'); } catch { console.log('[warn] nx build skipped'); }
  try { run('pnpm -w exec nx run api:test'); } catch { console.log('[warn] nx api:test skipped'); }
} else {
  console.log('[skip] tests');
}

console.log('[done] Orchestration complete ✅ (apiDir=%s, schema=%s)', apiDir, schema);
