...
const importRe =
  /^import\s+(type\s+)?\{\s*([^}]+)\s*\}\s+from\s+'@prisma\/client';\s*$/gm;
let m,
  names = new Set();
...
while (
  idx < lines.length &&
  (/^\s*(\/\/|\/\*|\*|\*\/|['"]use client['"];|['"]use server['"];)?\s*$/.test(
    lines[idx]
  ) ||
    lines[idx].trim() === '')
) {
...
lines.splice(
  idx,
  0,
  `import { ${Array.from(names).sort().join(', ')} } from '@prisma/client';`
);
s = lines.join('\n');
...
console.log(
  '✓ Deduped @prisma/client imports → single line with { Prisma, $Enums }'
);
