...
for (const m of MODS) {
  ...
  if (exists(e2e)) {
    tagged += ensureHeaderIf(
      e2e,
      (s) => /placeholder/.test(s) || /expect\(true\)\.toBe\(true\)/.test(s)
    )
      ? 1
      : 0;
  }
  ...
  // Types stubs
  const typesDir = exists(P.sharedTypesSrcDir)
    ? P.sharedTypesSrcDir
    : P.sharedTypesDir;
  ...
  if (exists(tf)) {
    tagged += ensureHeaderIf(
      tf,
      (s) =>
        /export type\s+[A-Z][A-Za-z0-9]*DTO/.test(s) &&
        s.split('\n').length < 60
    )
      ? 1
      : 0;
...
console.log(
  tagged
    ? `✓ Tagged ${tagged} stub file(s) with STUB:PHASE3`
    : '= No new stubs tagged'
);
