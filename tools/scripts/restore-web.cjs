...
function findArchived() {
  const candidates = [
    ...
    ...globNames(path.join(repo, 'apps'), /^web\._archived_/).map((n) =>
      path.join('apps', n)
    ),
  ]
    .sort()
...
function globNames(dir, regex) {
  ...
  return fs
    .readdirSync(dir)
    .filter(
      (n) => regex.test(n) && fs.statSync(path.join(dir, n)).isDirectory()
    );
}
...
function main() {
  ...
  if (!archived) {
    log(
      '⚠️  No web._archived_* folder found. Creating a fresh apps/web skeleton instead.'
    );
  } else {
  ...
  ensureFile(
    path.join(target, 'next.config.js'),
    `const path = require('path');
...
`
  ...
  ensureFile(
    ...
    `export default function Home(){return <main className="p-6">CareerOS Web — restored</main>};\n`
  );

  // TRPC wiring
  ensureFile(
    path.join(target, 'src', 'trpc', 'index.ts'),
...
`
  ...
  ensureFile(
    path.join(target, 'src', 'app', 'providers.tsx'),
    `'use client';
...
`
  ...
  ensureFile(
    path.join(target, 'vitest.setup.ts'),
    `vi.mock('@/trpc', () => {
