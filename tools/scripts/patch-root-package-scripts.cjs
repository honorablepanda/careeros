...
j.scripts["test:api"] = j.scripts["test:api"] ||
  'pnpm -w jest --clearCache --config apps/api/jest.config.ts && pnpm -w exec nx run api:test --verbose';
j.scripts["test:api:guarded"] = j.scripts["test:api:guarded"] ||
